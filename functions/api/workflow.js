...
async function readDoc(env) {
  ...
  doc.bhstatus = doc.bhstatus || {};   // Silk pipeline-statussen per account-id
  return doc;
}
...
export async function onRequestPost({ request, env }) {
  ...
  } else if (body.action === "bhstatus" && body.id != null) {
    // Silk: pipeline-status per account. status = followup|meeting|notnow|notinterested,
    // alles anders (null, "") wist de status.
    const k = String(body.id).slice(0, 80);
    const valid = ["followup", "meeting", "notnow", "notinterested"];
    if (body.status && valid.includes(body.status)) {
      doc.bhstatus[k] = { s: body.status, date: at.slice(0, 10) };
    } else {
      delete doc.bhstatus[k];
    }
  } else {
